...
hexo.extend.helper.register('_cover', function(item, num) {
  const { statics, js, image_server, image_list, covers } = hexo.theme.config;

  // 检查是否是首页的轮播图（通过num参数判断，首页轮播图通常需要多张图片）
  if (num && num > 1 && covers && covers.length > 0) {
    // 首页轮播图使用covers配置项中的图片
    const result = [];
    for (let i = 0; i < num; i++) {
      result.push(url_for.call(this, covers[i % covers.length]));
    }
    return result;
  } 
  // 对于文章列表中的图片，使用文章自身的cover或photos，或者随机背景
  else if(item.cover) {
